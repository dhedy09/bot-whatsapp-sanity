# Gunakan base image Node.js versi 18
FROM node:18-slim

# Install semua dependensi sistem yang dibutuhkan oleh Puppeteer/Chromium
# Ini sama seperti yang kita coba lakukan dengan Nix di Replit, tapi lebih pasti
RUN apt-get update && apt-get install -y \
    ca-certificates \
    fonts-liberation \
    libasound2 \
    libatk-bridge2.0-0 \
    libatk1.0-0 \
    libc6 \
    libcairo2 \
    libcups2 \
    libdbus-1-3 \
    libexpat1 \
    libfontconfig1 \
    libgbm1 \
    libgcc1 \
    libglib2.0-0 \
    libgtk-3-0 \
    libnspr4 \
    libnss3 \
    libpango-1.0-0 \
    libpangocairo-1.0-0 \
    libstdc++6 \
    libx11-6 \
    libx11-xcb1 \
    libxcb1 \
    libxcomposite1 \
    libxcursor1 \
    libxdamage1 \
    libxext6 \
    libxfixes3 \
    libxi6 \
    libxrandr2 \
    libxrender1 \
    libxss1 \
    libxtst6 \
    lsb-release \
    wget \
    xdg-utils \
    --no-install-recommends

# Tentukan folder kerja di dalam container
WORKDIR /usr/src/app

# Salin file package.json dan package-lock.json terlebih dahulu
COPY package*.json ./

# Install semua dependensi Node.js
RUN npm install --production

# Salin sisa kode proyek Anda ke dalam container
COPY . .

# Perintah untuk menjalankan aplikasi Anda
CMD ["node", "index.js"]